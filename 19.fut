...
let parse (l: []i32): instruction =
  {op=all_ops[l[0]], a=u32.i32 l[1], b=u32.i32 l[2], c=u32.i32 l[3]}
...
let step_until_r0_increased (m: machine) =
  let r0 = m.r.r0
  in loop (m, i) = (m, 0i32) while m.r.r0 == r0 do (step m, i+1)

entry steps (steps: i32) (ipr: i32) (input: [][]i32) =
  let code = map parse input
...
entry codehisto (steps: i32) (ipr: i32) (input: [][]i32) =
  let code = map parse input
...
entry part2 (y: i32) = map (\x -> if y % x == 0 then x else 0) (1...y) |> i32.sum
