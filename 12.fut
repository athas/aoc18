...
let evolve [n] (patterns: patterns) (state: [n]bool) =
  let get i = if i < 0 || i >= length state then 0 else i64.bool state[i]
  let new_cell i = patterns[pattern [get (i-2), get (i-1), get i, get (i+1), get (i+2)]]
  in tabulate n new_cell

let sum_live_pots [n] (shift: i64) (state: [n]bool) =
  map2 (*) (map i64.bool state) (map (+shift) (iota n |> map i64.i64))
  |> i64.sum

entry part1 (initial_raw: []i64) (patterns_raw: [][6]i64) =
  let patterns = parse_patterns patterns_raw
  let generations = 20
  let initial = replicate generations false ++
                map bool.i64 initial_raw ++
                replicate generations false
  let final_state = loop s = initial for _i < generations do evolve patterns s
  in sum_live_pots (i64.i64 (-generations)) final_state

type~ state = {shift: i64, cells: []bool}

let state_match [n] (x: [n]bool) (y: [n]bool) =
  let yget r i = let j = r+i
                 in if j < 0 || j >= n then false else y[j]
  in map3 (\b x y -> if b then x else y)
          (tabulate (n-1) (\r -> map2 (==) x (map (yget (r+1)) (iota n)) |> and))
          (map (+1) (iota (n-1)))
          (replicate (n-1) i64.highest)
      |> i64.minimum

-- WHACK ASSUMPTION: a cycle can at most only move by one cell.
entry part2 (initial_raw: []i64) (patterns_raw: [][6]i64) =
  let patterns = parse_patterns patterns_raw
  ...
  let shift = generations - i64.i64 first_repeat - i64.i64 basic_shift
